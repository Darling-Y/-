<template>
  <div class="wrapper">
    ...
    <swiper class="swiper"
      ...
      <block v-for="(item, index) in goods.pics"
             :key="index">
    ...
    <div class="fixed-bottom">
      ...
      <div class="icon-text"  @click="toCart">
        <span class="iconfont icon-icon22fuzhi"></span>
        <span>购物车</span>
      </div>
...
export default {
  ...
  // 在button按钮上添加open-type="share"
  // 会自动触发onShareAppMessage事件
  // 自定义转发内容
  onShareAppMessage () {
    return {
  ...
  methods: {
    // 点击加入到本地缓存,购物车里数据直接从里取
    add2Cart () {
      // 创建一个对象用来存商品信息
      let cart = wx.getStorageSync('cart') || {}
      let goodsId = this.goods.goods_id
      cart[goodsId] = {
        // 当此商品存在的时候,里面的num++,不存在就等于1
